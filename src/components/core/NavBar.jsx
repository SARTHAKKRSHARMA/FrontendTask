...
const NavBar = () => {
    ...
    const onClickHandler = (newCurrPage) => {
        if(currPage !== newCurrPage)
        {
            ...
            let filteredPage = [];
            if(newCurrPage === 'all') filteredPage = callList.filter((item) => item?.to && item?.from && item?.via  &&  item?.is_archived === false);
            else if (newCurrPage === "inbox")
            {
                filteredPage = callList.filter((item)=> item?.to && item?.from && item?.via  &&  item?.is_archived === false && item?.created_at && item?.direction==="inbound");
            }
            else
            {
                filteredPage = callList.filter((item) => item?.to && item?.from && item?.via  &&   item?.created_at && item?.is_archived);
                setIsVisible(false);
            }
